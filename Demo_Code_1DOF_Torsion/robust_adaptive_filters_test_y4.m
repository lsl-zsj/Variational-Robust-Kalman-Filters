function robust_adaptive_filters_test_y4()
% Brief: This codes demonstrates the ability of STKF-AR1 to handle
% time-varying measurement noise and process noise contaminated by
% outliers.
% Details:
%    None
% 
% Syntax:  
%     robust_adaptive_filters_test_y4()
% 
% Inputs:
%    None
% 
...
%% outliers in the process and adaptive measurement
...
uk=0*ones(1,len);
[state,z,d,amp]=measurement_generation_y4(kfa,uk);
stateaug=[d;state];
...
rho_r=0.98*ones(2,1);
kfa.dt=dt;
stakf=student_forward_adap_y2(kfa,uk,z,rho_p,rho_r);

% figure
% plot(t,stakf.Lambda','LineWidth',1)
...
figure 
plot(t,vbkff.Lambda','LineWidth',1)
hold on
plot(t,amp.*amp*Vvn,'LineWidth',1)
xlabel('Time (s)','Interpreter', 'latex');
ylabel('Value','Interpreter', 'latex');
grid on;
hold off;
set(gca,'fontsize',14)
legend('VBKF ${\sigma_{v_1}}^2$', 'VBKF ${\sigma_{v_2}}^2$','Truth','Interpreter', 'latex',...
     'Orientation','horizontal');



figure
plot(t,stakf.Lambda(6:7,:)'*Vvn,'LineWidth',1)
hold on
plot(t,amp.*amp*Vvn,'LineWidth',1)
xlabel('Time (s)','Interpreter', 'latex');
ylabel('Value','Interpreter', 'latex');
...
hold off;
set(gca,'fontsize',14)
legend('STKF-AR1 ${\sigma_{v_1}}^2$', 'STKF-AR1 ${\sigma_{v_2}}^2$','Truth','Interpreter', 'latex',...
    'Orientation','horizontal');
